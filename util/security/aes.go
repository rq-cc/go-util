...
// CFB模式的AES加密字符串（统一函数�?func AESCFBEncryptString(plaintext, key string) (string, error) {
...
// CFB模式的AES解密字符串（统一函数�?func AESCFBDecryptString(ciphertext, key string) (string, error) {
	normalizedKey := normalizeKey([]byte(key))
	...
	if err != nil {
		return "", fmt.Errorf("解码十六进制字符串失�? %v", err)
	}
...
// PKCS7去填�?func pkcs7UnPadding(data []byte) []byte {
	length := len(data)
	if length == 0 {
		return data
	}
